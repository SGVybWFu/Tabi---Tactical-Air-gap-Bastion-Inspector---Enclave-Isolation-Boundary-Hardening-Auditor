...
Linux: tabi (standalone ELF binary for x86_64 Linux distributions)
...
The compiled executable is published as a verified artifact in the GitHub repository's Releases tab (v1.1.0).

An operator downloads the binary onto an approved transfer medium (e.g., a CD-R disc or a flash drive equipped with a hardware Write-Protect switch).

On the target field terminal, run directly:
...
Preparing a Portable USB Package (TABI_PORTABLE):
...
Contents of the launcher batch script RUN_TABI.bat:
...
Write-Once-Read-Many (WORM) Transfer Media: Supplying audit tooling on finalized CD-R/DVD-R media or flash drives with physical write-lock toggles prevents malicious hosts from tampering with or infecting the auditor toolset.

Cryptographic Verification (SHA-256): Always verify the SHA-256 hash of the binary prior to execution on an operational asset:
...
Passive / Stealth Mode (--quick): On workstations operating under strict radio silence or emission security policies, run TABI with the -q / --quick flag to eliminate all outbound network probes and test strictly local boundary controls.
