# **TABI — Field Deployment & Air-Gap Operations Guide**
...
> Upstream Python 3.9+ dropped support for Windows 7\. To ensure the compiled TABI Tactical Air-gap & Bastion Inspector.exe executes seamlessly on legacy Windows 7 / Embedded Standard 7 terminals as well as modern Windows 11 systems, **build the executable on an engineering workstation equipped with Python 3.8.10 (32-bit or 64-bit)**.
...
\# 1\. Build a single standalone .exe for Windows (with console, embedded logo, and full designation):  
python \-m PyInstaller \--onefile \--console \--clean \--icon="logo.ico" \--name "TABI Tactical Air-gap & Bastion Inspector" tabi.py

\# 2\. Build a standalone ELF binary on Linux (Kali / Debian / RHEL):  
pyinstaller \--onefile \--name tabi tabi.py

The resulting file (dist/TABI Tactical Air-gap & Bastion Inspector.exe or dist/tabi) is roughly 9–12 MB in size.

### **Field Distribution Workflow:**

1. The compiled executable (TABI Tactical Air-gap & Bastion Inspector.exe / tabi) is published as a verified artifact in the GitHub repository's **Releases** tab (v1.0).  
...
Field operators simply plug in the media and double-click RUN\_TABI.bat.
...
2. **Cryptographic Verification:** Always verify the SHA-256 hash of the binary prior to execution on an operational asset:  
