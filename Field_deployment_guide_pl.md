# **TABI — Wdrażanie w Środowiskach Polowych i Odciętych (Air-Gap Deployment Guide)**
...
\# Na stacji inżynierskiej z Pythonem:  
python \-m pip install pyinstaller

\# 1\. Budowa pojedynczego pliku .exe dla Windows (z ikoną i pełną nazwą):  
python \-m PyInstaller \--onefile \--console \--clean \--icon="TABILogo.ico" \--name "TABI Tactical Air-gap & Bastion Inspector" tabi.py

\# 2\. Budowa pojedynczego pliku binarnego dla Linux:  
...
1. Gotowy plik TABI Tactical Air-gap & Bastion Inspector.exe umieszczono w sekcji **Releases** na GitHubie (v1.0).  
...
Operator w terenie jedynie podłącza nośnik i klika URUCHOM\_TABI.bat.
